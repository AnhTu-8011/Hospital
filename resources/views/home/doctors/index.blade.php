    ...
    <section class="py-5 py-lg-6" style="background: linear-gradient(135deg, #e3f2ff 0%, #f6fbff 40%, #ffffff 100%);">
        <div class="container">
            <div class="text-center mb-5">
                <p class="text-uppercase text-primary fw-semibold mb-2" style="letter-spacing: .08em;">ĐỘI NGŨ Y BÁC SĨ</p>
                                    ...
                                    <div class="position-relative mb-3">
                                        ...
                                        <div class="position-absolute top-0 end-0 m-2">
                                            <span class="badge bg-primary-subtle text-primary rounded-pill px-2 py-1">
                                                <i class="bi bi-check-circle-fill me-1"></i>Chuyên nghiệp
                                            </span>
                                        </div>
                                    </div>
                                    <h5 class="fw-bold mb-1 text-dark">{{ $name }}</h5>
                                    <p class="text-primary small mb-1 fw-semibold">
                                        <i class="bi bi-hospital me-1"></i>{{ $deptName }}
                                    </p>
                                    @if($spec)
                                        <p class="text-muted small mb-3">
                                            <i class="bi bi-award me-1"></i>{{ $spec }}
                                        </p>
                                    @endif
                                    <a href="#" class="btn btn-outline-primary btn-sm rounded-pill px-3" 
                                       data-bs-toggle="modal" data-bs-target="#doctorModal{{ $doc->id }}">
                                        <i class="bi bi-info-circle me-1"></i> Hồ sơ chi tiết
                                    </a>
                                </div>
                            </div>
                                    ...
                                    <div class="modal-content rounded-4 border-0 shadow-lg">
                                        <div class="modal-header border-0 pb-0" style="background: linear-gradient(135deg, #e3f2ff 0%, #f6fbff 100%); border-radius: 1rem 1rem 0 0 !important;">
                                            <h5 class="modal-title fw-bold text-primary" id="doctorModalLabel{{ $doc->id }}">
                                                <i class="bi bi-person-badge me-2"></i>Hồ sơ chi tiết - Bác Sĩ {{ $name }}
                                            </h5>
                                                    ...
                                                    <div class="position-relative">
                                                        ...
                                                        <div class="position-absolute top-0 end-0 m-2">
                                                            <span class="badge bg-primary rounded-pill px-3 py-2">
                                                                <i class="bi bi-check-circle-fill me-1"></i>Bác sĩ
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-8">
                                                    <div class="bg-light rounded-3 p-3 mb-3">
                                                        <p class="mb-2">
                                                            ...
                                                            <strong>Khoa:</strong> <span class="text-dark">{{ $deptName }}</span>
                                                        </p>
                                                    @if($spec)
                                                            <p class="mb-2">
                                                                <i class="bi bi-award text-primary me-2"></i>
                                                                <strong>Chuyên môn:</strong> <span class="text-dark">{{ $spec }}</span>
                                                            </p>
                                                    @endif
                                                    @if(!empty($doc->birth_date))
                                                            <p class="mb-0">
                                                                <i class="bi bi-calendar3 text-primary me-2"></i>
                                                                <strong>Ngày sinh:</strong> <span class="text-dark">{{ \Carbon\Carbon::parse($doc->birth_date)->format('d/m/Y') }}</span>
                                                            </p>
                                                    @else
                                                            <p class="mb-0">
                                                                <i class="bi bi-calendar3 text-primary me-2"></i>
                                                                <strong>Ngày sinh:</strong> <span class="text-muted">Chưa cập nhật.</span>
                                                            </p>
                                                    @endif
                                                    </div>
                                                    @if(!empty($doc->description))
                                                        <div class="mb-3">
                                                            <h6 class="fw-bold text-primary mb-2">
                                                                <i class="bi bi-file-text me-2"></i>Giới thiệu
                                                            </h6>
                                                            <div class="bg-light rounded-3 p-3">
                                                                <p class="mb-0" style="white-space: pre-line; color: #495057; line-height: 1.7;">{{ $doc->description }}</p>
                                                            </div>
                                                        </div>
                                                    @else
                                                        <div class="alert alert-info rounded-3 border-0 mb-3">
                                                            <i class="bi bi-info-circle me-2"></i>Chưa có mô tả.
                                                        </div>
                                                    @endif
                                                    ...
                                                    @if(!empty($doc->license_image))
                                                        <div class="mt-3">
                                                            <h6 class="fw-bold text-primary mb-3">
                                                                <i class="bi bi-id-card text-primary me-2"></i>Ảnh giấy phép hành nghề
                                                            </h6>
                                                    ...
                                                    @else
                                                        <div class="alert alert-info rounded-3 border-0 mt-3">
                                                            <i class="bi bi-id-card me-2"></i>Chưa có ảnh giấy phép hành nghề.
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
