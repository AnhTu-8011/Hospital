    ...
    <section class="py-5 py-lg-6" style="background: linear-gradient(135deg, #e3f2ff 0%, #f6fbff 40%, #ffffff 100%);">
        <div class="container">
            <div class="text-center mb-5">
                <p class="text-uppercase text-primary fw-semibold mb-2" style="letter-spacing: .08em;">CHUYÊN KHOA</p>
                    ...
                    <form action="{{ route('departments.index') }}" method="GET" class="row g-3 align-items-end">
                        <div class="col-md-10">
                            <label for="symptom" class="form-label fw-semibold mb-2">
                                <i class="bi bi-search text-primary me-2"></i>Tìm kiếm theo triệu chứng:
                            </label>
                            <input type="text" 
                                   name="symptom" 
                                   id="symptom" 
                        ...
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-primary btn-lg w-100 rounded-3 shadow-sm" style="background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%); transition: all 0.3s ease;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(13, 110, 253, 0.4)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 8px rgba(13, 110, 253, 0.3)';">
                        ...
                        @foreach($departments as $dept)
                            ...
                            <div class="col-md-4 col-sm-6">
                                <div class="card border-0 h-100 bg-white rounded-4 shadow-sm" 
                                     ...
                                     data-bs-toggle="modal" data-bs-target="#departmentModal{{ $dept->id }}">
                                ...
                                <div class="modal-dialog modal-xl modal-dialog-centered" style="max-width:90%;">
                                    <div class="modal-content rounded-4 border-0 shadow-lg">
                                        <div class="modal-header border-0 pb-0" style="background: linear-gradient(135deg, #e3f2ff 0%, #f6fbff 100%); border-radius: 1rem 1rem 0 0 !important;">
                                            <h5 class="modal-title fw-bold text-primary" id="departmentModalLabel{{ $dept->id }}">{{ $dept->name }}</h5>
                                                ...
                                                <div class="col-md-5">
                                                    <div class="bg-light rounded-3 p-3 h-100">
                                                        <h6 class="fw-bold mb-3 text-primary">
                                                            <i class="bi bi-info-circle me-2"></i>Mô tả chuyên khoa
                                                        </h6>
                                                        <p class="mb-0" style="white-space: pre-line; color: #495057; line-height: 1.7;">{{ $dept->description ?? 'Chưa có mô tả cho chuyên khoa này.' }}</p>
                                                    </div>
                                                </div>
                                                <div class="col-md-7">
                                                    <h6 class="fw-bold mb-3 text-primary">
                                                        <i class="bi bi-list-ul me-2"></i>Danh sách dịch vụ của khoa
                                                    </h6>
                                                    @if($deptServices->count())
                                                        <div class="list-group mb-0">
                                                            @foreach($deptServices as $service)
                                                                <div class="list-group-item border-0 rounded-3 mb-2 shadow-sm" style="transition: all 0.3s ease;"
                                                                    ...
                                                                    <div class="me-3" style="width:60px; height:60px; flex-shrink:0;">
                                                                        @if(!empty($service->image))
                                                                                <img src="{{ asset('storage/'.$service->image) }}" alt="{{ $service->name }}" 
                                                                                     class="rounded" style="width: 60px; height: 60px; object-fit: cover; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
                                                                        @else
                                                                    ...
                                                                    <div class="flex-grow-1 d-flex justify-content-between align-items-start">
                                                                        <div class="me-3">
                                                                            ...
                                                                            @if(!empty($service->description))
                                                                                    <div class="small text-muted" style="white-space: pre-line; line-height: 1.5;">{{ \Illuminate\Support\Str::limit($service->description, 80) }}</div>
                                                                                @endif
                                                                            </div>
                                                                            @if(!is_null($service->price))
                                                                                <span class="badge bg-primary rounded-pill px-3 py-2">{{ number_format($service->price, 0, ',', '.') }} đ</span>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                    ...
                                                    @else
                                                        <div class="alert alert-info mb-0 rounded-3 border-0">
                                                            <i class="bi bi-info-circle me-2"></i>Chưa có dịch vụ nào được gán cho khoa này.
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                        ...
                                        <div class="modal-footer border-0 pt-0">
                                            ...
                                            <a href="{{ route('services.index') }}" class="btn btn-primary rounded-pill px-4 shadow-sm">
                                                <i class="bi bi-arrow-right me-2"></i>Xem tất cả dịch vụ
                                            </a>
                                        </div>
                                    </div>
                                </div>
