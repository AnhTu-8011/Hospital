    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <div>
            ...
            <h1 class="h3 mb-0 fw-bold text-dark">
                <i class="fas fa-edit me-2 text-primary"></i>Sửa bệnh
            </h1>
        </div>
            ...
            <form action="{{ route('admin.diseases.update', $disease) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <select name="department_id" class="form-select rounded-3 border-2 @error('department_id') is-invalid @enderror" required>
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" {{ old('department_id', $disease->department_id) == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        @if($disease->image)
                            <div class="mt-2">
                                <img src="{{ asset('storage/'.$disease->image) }}" alt="{{ $disease->name }}" style="height: 64px;border-radius:8px;">
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="6" class="form-control rounded-3 border-2 @error('description') is-invalid @enderror">{{ old('description', $disease->description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <div id="symptoms-container">
                        @php $oldSymptoms = old('symptoms'); @endphp
                        @if(is_array($oldSymptoms))
                            @foreach($oldSymptoms as $sym)
                                <div class="input-group mb-2">
                                    <input type="text" name="symptoms[]" value="{{ $sym }}" class="form-control rounded-3 border-2 symptom-input" placeholder="Nhập triệu chứng (ví dụ: đau đầu, sốt, ho...)">
                                    <button type="button" class="btn btn-outline-danger rounded-end-3 remove-symptom"><i class="fas fa-times"></i></button>
                                </div>
                            @endforeach
                        @elseif($disease->symptoms && $disease->symptoms->count())
                            @foreach($disease->symptoms as $symptom)
                                <div class="input-group mb-2">
                                    <input type="text" name="symptoms[]" value="{{ $symptom->symptom_name }}" class="form-control rounded-3 border-2 symptom-input" placeholder="Nhập triệu chứng (ví dụ: đau đầu, sốt, ho...)">
                                    <button type="button" class="btn btn-outline-danger rounded-end-3 remove-symptom"><i class="fas fa-times"></i></button>
                                </div>
                            @endforeach
                        @else
                            <div class="input-group mb-2">
                                ...
                                <button type="button" class="btn btn-outline-danger rounded-end-3 remove-symptom" style="display:none;"><i class="fas fa-times"></i></button>
                            </div>
                        @endif
                    </div>
                ...
                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-lg rounded-pill shadow-lg text-white fw-bold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                        <i class="fas fa-save me-2"></i>Cập nhật bệnh
                    </button>
