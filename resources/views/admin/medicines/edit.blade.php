    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <div>
            ...
            <h1 class="h3 mb-0 fw-bold text-dark">
                <i class="fas fa-pills me-2 text-primary"></i>Cập nhật thuốc
            </h1>
        </div>
        <div>
    ...
    <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
        <div class="card-header border-0 py-3 px-4" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            <h6 class="m-0 font-weight-bold text-white d-flex align-items-center">
                <i class="fas fa-edit me-2"></i>Thông tin thuốc
            </h6>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('admin.medicines.update', $medicine) }}" method="POST">
                ...
                @php
                    $old = function($field, $default = null) use ($medicine) {
                        return old($field, $medicine->{$field} ?? $default);
                    };
                @endphp
